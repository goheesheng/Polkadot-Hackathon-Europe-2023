import type * as EventTypes from '../event-types/rmrk_example_mintable';
...
export default class EventsClass {
	...
	public subscribeOnTransferEvent(callback : (event : EventTypes.Transfer) => void) {
		const callbackWrapper = (args: any[], event: any) => {
			...
			callback(handleEventReturn(_event, getEventTypeDescription('Transfer', 'rmrk_example_mintable')) as EventTypes.Transfer);
		};
	...
	public subscribeOnApprovalEvent(callback : (event : EventTypes.Approval) => void) {
		const callbackWrapper = (args: any[], event: any) => {
			...
			callback(handleEventReturn(_event, getEventTypeDescription('Approval', 'rmrk_example_mintable')) as EventTypes.Approval);
		};
