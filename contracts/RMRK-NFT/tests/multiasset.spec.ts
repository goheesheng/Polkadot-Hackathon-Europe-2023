...
describe("RMRK Multi Asset tests", () => {
  ...
  it("Add assets to token and approve them", async () => {
    ...
    await gem
      .withSigner(bob)
      .tx.acceptAsset({ u64: 1 }, 1, { gasLimit: assetAcceptGas * 2n });
    await gem
      .withSigner(bob)
      .tx.acceptAsset({ u64: 1 }, 2, { gasLimit: assetAcceptGas * 2n });
    await gem
      .withSigner(bob)
      .tx.acceptAsset({ u64: 1 }, 3, { gasLimit: assetAcceptGas * 2n });
    await gem
      .withSigner(bob)
      .tx.acceptAsset({ u64: 1 }, 4, { gasLimit: assetAcceptGas * 2n });
    await gem
      .withSigner(bob)
      .tx.acceptAsset({ u64: 2 }, 1, { gasLimit: assetAcceptGas * 2n });
    await gem
      .withSigner(bob)
      .tx.acceptAsset({ u64: 2 }, 2, { gasLimit: assetAcceptGas * 2n });
    await gem
      .withSigner(bob)
      .tx.acceptAsset({ u64: 2 }, 3, { gasLimit: assetAcceptGas * 2n });
    await gem
      .withSigner(bob)
      .tx.acceptAsset({ u64: 2 }, 4, { gasLimit: assetAcceptGas * 2n });
    await gem
      .withSigner(bob)
      .tx.acceptAsset({ u64: 3 }, 5, { gasLimit: assetAcceptGas * 2n });
    await gem
      .withSigner(bob)
      .tx.acceptAsset({ u64: 3 }, 6, { gasLimit: assetAcceptGas * 2n });
    await gem
      .withSigner(bob)
      .tx.acceptAsset({ u64: 3 }, 7, { gasLimit: assetAcceptGas * 2n });
    await gem
      .withSigner(bob)
      .tx.acceptAsset({ u64: 3 }, 8, { gasLimit: assetAcceptGas * 2n });
    expect(
      (await gem.query.totalTokenAssets({ u64: 1 }))?.value.ok.toString()
    ).to.be.equal("4,0");
    expect(
